...
Feature selection requires `vip` 0.5.0 or newer, which requires R 4.1 or newer and is distributed from its maintainer's r-universe repository. Install it before enabling feature selection:
...
FinnTS remains installable without `vip`, but requesting feature selection without it produces an actionable error before feature-selection models are fitted.
