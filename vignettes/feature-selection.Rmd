...
Uses the [Boruta](https://cran.r-project.org/web/packages/Boruta/index.html) package. It finds relevant features by comparing original attributes' importance with importance achievable at random, estimated using their permuted copies (shadows).

Boruta is not ran for daily or weekly data, in order to save time.

### Model Variable Importance

Multiple models (cubist, glmnet, xgboost) are trained on the entire training data. These models are then fed into the variable importance function within the [vip](https://koalaverse.github.io/vip/index.html) package. For each model, any feature with a variable importance score above zero is flagged as important.
