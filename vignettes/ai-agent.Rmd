...
- Evaluates future quality when `final_models()` selects the winner within each iteration, then compares completed iteration winners using WMAPE only. Equal accuracy retains the earlier winner, and recorded metrics avoid repeating past future-quality checks.
...
Reconciliation uses the selected forecasts and backtest residuals, not retained quality rankings. Incomplete selection retries rebuild averages and winner flags from existing predictions without retraining. Global updates preserve each series' saved winning subset and check the newly produced forecasts before acceptance; they do not reintroduce every model from the original request.
...
Within each Agent iteration, `final_models()` selects candidates using accuracy and future-quality checks. Completed iteration winners are compared by WMAPE only, retaining the earlier winner on equal accuracy. Normal accuracy-goal stopping requires a complete eligible result and finite WMAPE, without a second soft-quality veto. Hierarchical comparisons use reconciled backtests, and recorded run metrics avoid reassessing past future paths. Rejected evaluations still consume iteration budget. Newly generated update forecasts and default replacements retain the stricter acceptance checks described above.
