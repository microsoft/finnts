...
## Case Weights
Case weights can be applied to multivariate models that support them in tidymodels (cubist, glmnet, mars, xgboost). When turned on, importance weights are used to make a predictive model care more about recent observations than older ones. Importance weights are assigned to each observation based on an exponential decay function of the date. You can control the use of case weights within `prep_models()`. More information about how case weights are used in forecasting can be found in this [tidymodels blog post](https://www.tidymodels.org/learn/work/case-weights/).
