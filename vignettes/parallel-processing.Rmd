...
Every `iterate_forecast()` time-series combo receives independent driver and reasoning Chat objects with empty conversation history, whether combos run sequentially or in parallel. Parallel runs require ellmer 0.4.0 or later on the driver and every worker. Finn sends the configured Chats through the standard `foreach` serialization path, then creates the per-combo deep clones. This preserves separate providers, models, parameters, tools, and credentials without rebuilding Chat objects through internal ellmer APIs.

Install the same supported ellmer version in the driver and worker R libraries. With Azure OpenAI API-key authentication, the default ellmer 0.4 credential callback is serialized with the Chat, so worker task payloads must be treated as sensitive. A custom `credentials` callback that reads an environment variable or obtains a token at request time requires that credential source to be available from every worker process.
