...
- Custom `seasonal_period` values are supplied through `prep_models()` only. Validate one to three unique, finite numeric values greater than 1, pass them to `stlm-arima`, `stlm-ets`, and `tbats`, and continue logging a default `NULL` as `NA` rather than its resolved periods.
