...
- Agent duplicate-run checks compare effective settings rather than raw text. Resolve `NULL` defaults and canonicalize order-insensitive multipart models, regressors, recipes, lags, rolling windows, and seasonal periods before comparison; keep raw values unchanged in run logs.
- Custom `seasonal_period` values are supplied through `prep_models()` only. Validate one to three unique, finite numeric values greater than 1, pass them to `stlm-arima`, `stlm-ets`, and `tbats`, and continue logging a default `NULL` as `NA` rather than its resolved periods. Apply the same validation to `reason_inputs()` proposals before submission so invalid recommendations return to the LLM retry path; after all retries are exhausted, gracefully abort optimization and preserve the existing best forecast. Defaults must never contain 1 (`year` uses `c(2, 3)`); when `update_forecast()` replays a legacy run containing an invalid period, warn once and use cadence defaults rather than weakening validation or silently removing individual values.
- Character combo-variable values use one canonical identity across agentic and standard workflows. Call the shared `normalize_combo_values()` path before validation, `Combo` construction, hierarchy processing, or artifact writes in both `set_agent_info()` and `prep_data()`. Trim leading/trailing whitespace only, preserve internal spaces and missing values, leave numeric identifiers unchanged, and fail before writing when normalization creates a blank value or duplicate combo/date. Do not move this behavior into generic `hash_data()` because that would alter unrelated cached artifact names.
- EDA summaries must never send `Inf`, `-Inf`, or `NaN` to an LLM when an outlier date or distance-correlation group has no observations. Represent absent outlier dates explicitly and omit unavailable regressor-lag rankings while preserving raw EDA artifacts.
