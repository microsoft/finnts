...
- Keep custom `seasonal_period` handling in `prep_models()`: validate one to three unique finite values above 1, forward them to the STLM/TBATS workflows, and leave default `NULL` values as `NA` in raw run logs. Reject invalid agent proposals in `reason_inputs()` before submission and gracefully abort optimization after exhausted retries while preserving the existing best forecast. Keep every cadence default above 1 (`year` uses `c(2, 3)`), and make legacy update replays warn and fall back to cadence defaults without weakening validation.
