...
## Notes
- Agent code must treat `agent_info$llm` as an immutable Chat template and create a fresh, empty-history session per time-series combo and Q&A workflow. Parallel code must clone inside the series workflow after `foreach` dispatch, preserve the ellmer 0.4+ serialization path, and avoid internal ellmer APIs or provider credential mutation.
