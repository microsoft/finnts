...
# * send_chronos_request retry logic ----

make_chronos_mock_response <- function(status_code, body = "[]") {
  structure(
    list(
...
test_that("send_chronos_request retries connection errors and fails after 4 total attempts", {
  call_count <- 0L
  withr::with_envvar(
    c(CHRONOS_API_URL = "https://example.com", CHRONOS_API_TOKEN = "test-token"),
    {
...
test_that("send_chronos_request retries on HTTP 429 and succeeds on second attempt", {
  call_count <- 0L
    ...
    c(CHRONOS_API_URL = "https://example.com", CHRONOS_API_TOKEN = "test-token"),
    {
      local_mocked_bindings(
        POST = function(...) {
          call_count <<- call_count + 1L
          if (call_count == 1L) {
            make_chronos_mock_response(429L, '{"error": "rate limited"}')
          } else {
            make_chronos_mock_response(200L, success_body)
          }
        },
        .package = "httr"
      )
...
test_that("send_chronos_request retries on HTTP 500 and succeeds on second attempt", {
  call_count <- 0L
  success_body <- '[{"unique_id": "s1", "ds": "2026-01-01"}]'
  withr::with_envvar(
    c(CHRONOS_API_URL = "https://example.com", CHRONOS_API_TOKEN = "test-token"),
    {
      local_mocked_bindings(
        POST = function(...) {
          call_count <<- call_count + 1L
          if (call_count == 1L) {
            make_chronos_mock_response(500L, '{"error": "internal server error"}')
          } else {
            make_chronos_mock_response(200L, success_body)
          }
        },
...
test_that("send_chronos_request does not retry on non-retryable 4xx errors", {
  call_count <- 0L
  withr::with_envvar(
    c(CHRONOS_API_URL = "https://example.com", CHRONOS_API_TOKEN = "test-token"),
    {
      local_mocked_bindings(
        POST = function(...) {
          call_count <<- call_count + 1L
          make_chronos_mock_response(400L, '{"error": "bad request"}')
        },
        .package = "httr"
      )
      local_mocked_bindings(
...
test_that("send_chronos_request total attempts equals CHRONOS_MAX_RETRIES + 1 for persistent errors", {
  # with CHRONOS_MAX_RETRIES = 3, exactly 4 total attempts (1 original + 3 retries)
  call_count <- 0L
  withr::with_envvar(
    c(CHRONOS_API_URL = "https://example.com", CHRONOS_API_TOKEN = "test-token"),
    {
